...
#### 功能4：设置与配置
**用户流程**：点击设置 → 配置AI服务 → 选择主题 → 查看日志

| 功能点 | 描述 |
|--------|------|
| 自定义AI配置 | 支持配置多个AI服务提供商，每个可独立设置API地址、模型名称、API密钥 |
| 环境变量支持 | API密钥支持 `{{ENV_VAR}}` 格式，便于部署时使用环境变量 |
| 默认提供商 | 可设置默认使用的AI服务，当主服务不可用时自动切换 |
| 主题切换 | 提供蓝色、黄色、绿色三种主题选择，全局颜色跟随主题变化 |
| 日志查看器 | 实时显示分析过程中的所有操作和状态，支持清空日志 |
...
| 目标字段 | 支持的列名关键词 |
|----------|------------------|
| 视频标题 | 标题/题目/title/内容/文案/视频名称/作品标题/topic |
| 点赞数 | 点赞/likes/赞/获赞/点赞总数/点赞数量 |
| 评论数 | 评论/comments/评/评论量/评论数量 |
| 收藏数 | 收藏/saves/收/喜欢/收藏量/收藏总数 |
| 转发数 | 转发/shares/发/分享/转发量/转发总数 |
| 发布时间 | 发布时间/time/publish/date/日期/发布日期/datetime/时间戳 |
...
| 主题色 | 使用场景 | 色值 |
|--------|----------|------|
| 蓝色（默认） | 主要操作、选中状态、按钮 | #3b82f6 |
| 黄色 | 警告、提示、次要操作 | #eab308 |
| 绿色 | 成功、完成、通过 | #22c55e |
| 灰色 | 中性文本、边框、背景 | 灰度200-900 |
...
#### 布局规范
- **间距系统**：使用 4px 基础单位的倍数（4/8/12/16/20/24...）
- **圆角规范**：
  - 小元素：4px（按钮、输入框）
  - 卡片：8-12px
  - 大容器：16px
- **边框**：1px 实线，颜色 gray-200
- **阴影**：仅在必要区域使用轻微阴影 `shadow-sm`

#### 字体规范
- **标题**：font-semibold，字重 600
- **正文**：font-normal，字重 400
- **辅助文字**：text-sm（14px），text-xs（12px）

#### 交互规范
- **按钮样式**：
  - 主按钮：bg-gray-900 text-white hover:bg-gray-800
  - 次要按钮：bg-white text-gray-900 border border-gray-300 hover:bg-gray-50
  - 禁用：bg-gray-300 text-gray-500 cursor-not-allowed
- **输入框**：border border-gray-300，聚焦时 ring-2 ring-gray-900
- **反馈**：操作后立即给出视觉或文本反馈
